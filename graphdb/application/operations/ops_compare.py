...
class CompareOperations:
    ...
    # Method: Get random sample of tuples for the primary key
        ...
        # No.
            ...
            if partition_by in primary_keys:
                ...
                for colval in partition_column_possible_vals:
                    sql_query_stack += [
                        f"(SELECT {', '.join(primary_keys)} FROM {schema_name}.{table_name} WHERE {partition_by} = '{colval}' ORDER BY RAND() LIMIT {round(sample_size/len(partition_column_possible_vals))})",
                    ]
    ...
    # Method: Get a random sample of tuples for the unique key named 'uid'
    ...
    # Method: Get rows by primary key set for comparison
    ...
    # Method: Get rows by unique key 'uid' set for comparison
    ...
    def compare_all_tables_by_random_sampling(self, source_engine_name, source_schema_name, target_engine_name, target_schema_name, sample_size=1024):
        """
        Compare all tables in a database across two MySQL servers/schemas by random sampling.
        """
    ...
    # Method: Main implementation for comparing tables by metadata (row counts, engine, etc.)
    def compare_tables_by_metadata(self, source_engine_name, source_schema_name,
    ...
    # Method: Batch compare all tables in a database by metadata
    def compare_databases_by_metadata(self, source_engine_name, source_schema_name,
                    target_engine_name, target_schema_name,
...
if __name__ == "__main__":
    ...
    ops.compare_tables_by_random_sampling(
        ...
        source_table_name  = 'Index_D_Publication',
        target_engine_name = 'xaas_coresrv',
        target_schema_name = 'graphsearch_prod_mirror',
        target_table_name  = 'Index_D_Publication',
        sample_size        = 2048
    )
