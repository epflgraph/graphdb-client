...
class GraphDB():
    ...
    #-------------------------------------------------#
    def execute_query_in_chunks(self, engine_name, schema_name, table_name, query, has_filters=None, table_to_chunk=None, chunk_filter=None, chunk_size=1000000, row_id_name='row_id', show_progress=False, verbose=False, query_id=None, desc=None):
        ...
        # Log the target table so the user knows what is being chunked
        sysmsg.info(
            f"Discovering chunk boundaries for {schema_name}.{table_name} "
        ...
        progress_iter = zip(starts, ends)
        if show_progress:
            progress_iter = tqdm(progress_iter, total=len(starts), desc=desc)
    ...
    #-------------------------------------------------------#
    # Helpers: strip comments / split SQL file statements   #
    #-------------------------------------------------------#
    @staticmethod
    ...
    @staticmethod
    def _parse_update_target(query: str) -> Optional[tuple[Optional[str], str, Optional[str]]]:
        """
        Extract (schema_name, table_name, alias) from an UPDATE statement.
        Returns None if the query is not a simple UPDATE.
        """
        pattern = re.compile(
            r"^\s*UPDATE\s+"
            r"(?:`?([\w_]+)`?\.)?"
            ...
            r"(?=\s+(?:INNER|LEFT|RIGHT|FULL|CROSS|JOIN|SET|WHERE|;|$)))?",
            re.IGNORECASE,
        )
    ...
    #---------------------------------------------------------------#
    # Method: Execute a SQL script statement-by-statement in chunks #
    #---------------------------------------------------------------#
    def execute_sql_statements_in_chunks(
        self,
        engine_name: str,
        sql: str,
        ...
        """
        Strip comments, split ``sql`` into statements, and execute each one.
        UPDATE statements whose target table has a row_id are executed in chunks.
        DDL / non-UPDATE statements are executed via execute_query_in_shell.
        """
        if schema_mapping:
            for placeholder, real_name in schema_mapping.items():
                sql = sql.replace(f"[[{placeholder}]]", real_name)

        cleaned_sql = self._strip_sql_comments(sql)
        statements = self._split_sql_statements(cleaned_sql)

        for idx, stmt in enumerate(statements, start=1):
            stmt_id = f"{query_id or 'sql_chunk'}-{idx}"
            upper = stmt.upper()

            if not upper:
                continue

            # DDL: cannot be chunked safely, run through the shell
            if any(kw in upper for kw in ("CREATE", "DROP", "ALTER", "TRUNCATE")):
                if verbose:
                    print_sql(stmt, title=f"Executing DDL [{stmt_id}]")
                self.execute_query_in_shell(
            ...
            parsed = self._parse_update_target(stmt) if upper.startswith("UPDATE") else None
            if parsed is None:
                # Statement we don't know how to chunk; run as-is
                if verbose:
                    print_sql(stmt, title=f"Executing statement [{stmt_id}]")
                self.execute_query_in_shell(
                    engine_name=engine_name,
                    query=stmt + ";",
                    verbose=verbose,
            ...
            schema_name, table_name, alias = parsed
            if not schema_name:
                raise ValueError(
                    f"Could not determine schema for statement [{stmt_id}]: {stmt[:120]}"
                )

            target_label = f"{schema_name}.{table_name}"
            sysmsg.info(
                f"Executing statement {idx}/{len(statements)} in chunks: {target_label}"
            )

            row_id_name = f"{alias}.row_id" if alias else "row_id"
            has_filters = "WHERE" in upper

            if verbose:
                print_sql(stmt, title=f"Executing in chunks [{stmt_id}]")

            self.execute_query_in_chunks(
                engine_name=engine_name,
    ...
    #---------------------------------------------------------------#
    # Method: Execute a SQL file statement-by-statement in chunks   #
    #---------------------------------------------------------------#
    def execute_sql_file_in_chunks(
        self,
        engine_name: str,
        ...
        """
        Read a SQL file and execute its statements in row_id chunks.
        """
        ...
        self.execute_sql_statements_in_chunks(
            engine_name=engine_name,
            sql=sql,
            chunk_size=chunk_size,
